...
Mat boundary(Mat des, int maskSize) {
	...
	for (int y = 0; y < des.rows; y++)
	{
		...
		{
			bout.at<uchar>(y, x) = des.at<uchar>(y, x) - out.at<uchar>(y, x);
		}
	}


	imshow("After Erosion",out);
	waitKey();
	return bout;
...
int main() {
	...
	Mat src = imread("sam2.tif", CV_LOAD_IMAGE_GRAYSCALE);
	int maskSize;
	...
	imshow("Input", src);
	waitKey();
	Mat output = boundary(des, maskSize);
	imshow("After Boundary Extraction", output);
	waitKey();
